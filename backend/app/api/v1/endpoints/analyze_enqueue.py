...
@router.post("/enqueue")
async def analyze_enqueue(req: AnalyzeRequest, db_session: AsyncSession = db) -> dict:
    if not await validate_kanji_characters(req.name_sei, req.name_mei, db_session):
...
async def validate_kanji_characters(name_sei: str, name_mei: str, db_session: AsyncSession) -> bool:
